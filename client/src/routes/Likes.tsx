...
export default function Likes() {
  const [likedUsers, setLikedUsers] = useState<UserInfo[] | null>(null);
  ...
  useEffect(() => {
    if (user) {
      ...
      if (token) {
        getLikes({ actorUserId: user.id, token })
          .then((likes: UserInfo[]) => {
            setLoader(false);
            setLikedUsers(likes);
          })
...
export function UserProfileCard({ userInfo }: { userInfo: UserInfo }) {
  const { user } = useContext(UserContext);
